...
def compute_filter_kd_tree(fe):
    node_coords = fe.points  # shape = (num_nodes, dim)
    flex_node_coords = np.take(node_coords, fe.flex_inds, axis=0)
    flex_num_nodes = len(fe.flex_inds)
    ...
    kd_tree = scipy.spatial.KDTree(flex_node_coords)
    I, J, Vals = [], [], []

    for i in range(flex_num_nodes):
        dd, ii = kd_tree.query(flex_node_coords[i], k=20)
        vals = np.where(rmin - dd > 0., rmin - dd, 0.)
        I += [i] * len(ii)
        J += ii.tolist()
        Vals += vals.tolist()

    H_sp = scipy.sparse.csc_array((Vals, (I, J)), shape=(flex_num_nodes, flex_num_nodes))
    H = BCOO.from_scipy_sparse(H_sp).sort_indices()
    Hs = H.sum(1).todense()
    return H, Hs

def applyDensityFilter(ft, rho):
    """rho: shape = (num_nodes, 1)"""
    return ft['H'] @ rho / ft['Hs'][:, None]

def applySensitivityFilter(ft, rho, dJ, dvc):
    """dJ: shape = (num_nodes, 1), dvc: (num_constraints, num_nodes)"""
    rho_safe = np.maximum(1e-3, rho)
    dJ = ft['H'] @ (rho * dJ / rho_safe / ft['Hs'][:, None])
    dvc = ft['H'][None, :, :] @ (
        rho[None, :, :] * dvc / rho_safe[None, :, :] / ft['Hs'][None, :, None]
    )
    return dJ, dvc
...
def optimize(fe, rho_ini, optimizationParams, objectiveHandle, consHandle, numConstraints):
    ...
    while loop < optimizationParams['maxIters']:
        ...
        J, dJ = objectiveHandle(rho_physical, loop)
