...
class UnitCell(ABC):
    """Abstract base class for computational unit cells in finite element analysis.
    
    A unit cell represents a fundamental repeating element in computational mechanics,
    commonly used for periodic boundary conditions, homogenization, and multiscale
    analysis. This class provides the geometric and topological infrastructure for
    unit cell-based computations.
    
    The UnitCell class manages mesh geometry, boundary identification, and coordinate
    transformations essential for unit cell analysis. It supports N-dimensional unit
    cells and provides methods for identifying corners, edges, faces, and applying
    macroscopic displacements.
    
    Attributes:
        mesh (Mesh): The finite element mesh of the unit cell.
        cells (np.ndarray): Element connectivity matrix.
        ele_type (str): Element type identifier (e.g., 'HEX8', 'TET4').
        points (np.ndarray): Node coordinates with shape (num_nodes, spatial_dim).
        atol (float): Absolute tolerance for geometric comparisons.
        
    Properties:
        cell_centers (np.ndarray): Centers of all elements in the mesh.
        bounds (Tuple[np.ndarray, np.ndarray]): Lower and upper bounds of the unit cell.
        corners (np.ndarray): All corner points of the unit cell bounding box.
        corner_mask (np.ndarray): Boolean mask identifying corner nodes.
        edge_mask (np.ndarray): Boolean mask identifying edge nodes.
        face_mask (np.ndarray): Boolean mask identifying face nodes.
        
    Example:
        Implementing a concrete unit cell:
        ...
        >>> print(f"Unit cell has {len(corners)} corners")
        
    Note:
        This is an abstract class. Concrete implementations must define the
        mesh_build() method to specify how the unit cell mesh is constructed.
    """
    ...
    def apply_macro_displacement(
        self, macro_strain: np.ndarray, origin: Optional[np.ndarray] = None
    ) -> np.ndarray:
        """Apply macroscopic displacement field to all mesh nodes.
        
        Computes the displacement field resulting from a prescribed macroscopic
        strain tensor applied to the unit cell. This is fundamental for multiscale
        analysis and homogenization where macroscopic deformation is imposed on
        the unit cell.
        
        The displacement is computed as: u = (x - x0) @ strain^T
        where x0 is the reference origin.
        
        Args:
            macro_strain (np.ndarray): Macroscopic strain tensor with shape (N, N)
                where N is the spatial dimension. For 3D, this is typically a
                3x3 strain tensor [ε_xx, ε_xy, ε_xz; ...]
            origin (np.ndarray, optional): Reference point for displacement calculation.
                If None, uses the lower bound of the unit cell. Shape: (N,)
        
        Returns:
            np.ndarray: Flattened displacement vector for all nodes.
                Shape: (num_nodes * spatial_dim,)
                
        Raises:
            ValueError: If dimensions of macro_strain don't match mesh dimensionality.
        
        Example:
            >>> # Apply 1% strain in x-direction for 3D unit cell
            >>> strain = np.array([[0.01, 0, 0], [0, 0, 0], [0, 0, 0]])
            >>> displacements = unit_cell.apply_macro_displacement(strain)
            >>> print(f"Applied displacement vector length: {len(displacements)}")
        """
        if self.points.shape[1] != macro_strain.shape[0]:
            raise ValueError(
                f"Dimension mismatch: points {self.points.shape[1]} vs macro_strain {macro_strain.shape[0]}"
            )
        if origin is None:
            origin = self.lb
        x_shifted = self.points - origin

        flatten_x_shifted = (x_shifted @ macro_strain.T).reshape(-1)
        return flatten_x_shifted
